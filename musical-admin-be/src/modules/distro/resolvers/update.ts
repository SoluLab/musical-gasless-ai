	...
	Mutation: {
		async distroStatus(
			...
			try {
				...
				const type = status === 'APPROVED' ? "DISTRO_APPROVED" : "DISTRO_REJECTED"

				//notification generate
				const notify = {
					type: type,
					from: updatedStatus.userId,
					to: updatedStatus.userId,
					resource: id.toString(),
					viewed: false,
					data: {
						distroId: id.toString()
					}
				}

				await App.Models.Notify.create(notify)
