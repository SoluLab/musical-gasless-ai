...
const canReleasePermission = rule()((__: any, ___: any, { user }) => {
	Logger.info('Inside canReleasePermission Middleware')
	return checkPermission(user, 7000)
})

const canQuestPermission = rule()((__: any, ___: any, { user }) => {
	Logger.info('Inside canQuestPermission Middleware')
	return checkPermission(user, 8000)
})

const canGamificationPermission = rule()((__: any, ___: any, { user }) => {
	Logger.info('Inside canGamificationPermission Middleware')
	return checkPermission(user, 9000)
})
...
const rateLimitRule = createRateLimitRule({
	identifyContext: (ctx) => {
		const id = ctx?.user?._id?.toString() ?? 'anonymous';
		return id;
	},
});
...
export default shield(
	{
		Query: {
			...
			questList: and(rateLimitRule(rateLimiterOption), canQuestPermission),
			quest: and(rateLimitRule(rateLimiterOption), canQuestPermission),

			gamificationList: and(rateLimitRule(rateLimiterOption), canGamificationPermission),
			gamification: and(rateLimitRule(rateLimiterOption), canGamificationPermission),
		...
		Mutation: {
			...
			createQuest: and(rateLimitRule(rateLimiterOption), canQuestPermission),
			updateQuest: and(rateLimitRule(rateLimiterOption), canQuestPermission),
			questStatus: and(rateLimitRule(rateLimiterOption), canQuestPermission),

			createEvent: and(rateLimitRule(rateLimiterOption), canGamificationPermission),
			updateEvent: and(rateLimitRule(rateLimiterOption), canGamificationPermission),
			updateStatus: and(rateLimitRule(rateLimiterOption), canGamificationPermission),
